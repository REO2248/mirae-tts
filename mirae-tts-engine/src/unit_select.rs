...
impl UnitSelection {
    pub fn active_data(&self) -> VoiceInfoEntry {
        match self.data2 {
            Some(d2) if self.marker2.is_some_and(|m| m % 10000 != 0) => d2,
            _ => self.data,
        }
    }
...
impl<'a> UnitSelector<'a> {
    ...
    pub fn process(&mut self, records: &[ProsodyRecord]) -> ProcessedUnits {
                ...
                if FALLBACK_ALLOW[mid5] == 0 {
                    ...
                    if hit.is_some() {
                        ...
                        if FALLBACK_ALLOW[mid5b] == 0 || (req_a.cur & 0xfc00) != 0x6c00 {
                            ...
                            let cur_hi = ((b | 0x360) << 5) | (req_a.cur & 0x1f);
        ...
        for u in units.iter_mut() {
            ...
            if is_real_phoneme(cur_hi, next_lo)
                && d.class_byte() % 10 < 2
                && let Some(extra) = self.scan_special(d.pitch as i16)
            {
                u.extra = Some(extra);
            ...
            if (cur_hi == 0 || cur_hi == 5 || cur_hi == 0xf) && !(cur_hi == 5 && next_lo == 0x10) {
                pause += 1000;
            }
            let cls = req.class;
