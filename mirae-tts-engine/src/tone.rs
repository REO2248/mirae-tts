...
// Canonical table lives in crate::tables (Future.exe .data @0x48bd40); re-export here.
pub use crate::tables::TONE_CLASS_MAP;
...
pub fn normalize_class(cls: u8) -> Option<u8> {
    (0..16u8).find(|&i| TONE_CLASS_MAP[usize::from(i) * 16] == cls)
}
...
mod tests {
    ...
    fn sandhi_single_record_sentence() {
        ...
        apply_sandhi(&mut buf, &mut s1);
        assert_eq!(buf[0].tone_class, 1 + 0x28);
    ...
    fn sandhi_first_record_no_previous_adjustment() {
        ...
        apply_sandhi(&mut buf, &mut s1);
        assert_eq!(buf[0].tone_class, 4 + 0x28);
    }
}
