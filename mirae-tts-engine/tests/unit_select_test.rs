...
fn voice_dir() -> Option<std::path::PathBuf> {
    common::voice_dir()
}
...
fn scan_matches_independent_python_reference() {
    ...
    type Case = ((u16, u16, u16, u16, u8, u8), (u32, u32, u16, i32));
    let cases: [Case; 5] = [
        (
...
fn boundary_code_used_for_prev_next() {
    ...
    let boundary = mirae_tts_engine::unit_select::BOUNDARY_CODE;
    assert_eq!(boundary, 0x6EB3);
    assert_eq!(out.units[0].request.prev, boundary, "u0 文頭 prev");
    ...
    assert_eq!(out.units[3].request.next, boundary, "u3 調値>1 next");
    assert_eq!(out.units[3].request.prev, 0x6d86);
    assert_eq!(out.units[4].request.prev, boundary, "u4 レベル>1 prev");
    assert_eq!(out.units[4].request.next, boundary, "u4 末尾 next");
}
...
fn sentence_end_record_gets_boundary_next() {
    ...
    let boundary = mirae_tts_engine::unit_select::BOUNDARY_CODE;
    assert_eq!(
        out.units[1].request.next, boundary,
        "文末レコード next は境界コード"
    );
    assert_eq!(out.units[2].request.prev, 0x6d80);
}
...
fn request_pitch_smoothing_runs() {
    ...
    for u in &out.units {
        assert!(u.data.pause >= 0 || u.data2.is_some_and(|d| d.pause >= 0));
    }
}
