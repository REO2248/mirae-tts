...
/// binding the directory as `voice_dir`.
macro_rules! require_voice {
    ($dir:ident) => {
        let $dir = match common::voice_dir() {
            Some(d) => d,
            None => {
...
fn parse_all_pkgs_exact_header_and_full_consumption() {
    require_voice!(voice_dir);
    /// (name, n1, n2, sub_a_count, sub_a_pairs, sub_b_count, sub_b_pairs, file size)
    type PkgCase = (
        &'static str,
        usize,
        usize,
...
fn alphabet_ga_reaches_node_522_tail_offset_64() {
    require_voice!(voice_dir);
    let d = load(&voice_dir, "Alphabet.pkg");
...
fn colligation_113910_terminal_entries_x_within_sub_a() {
    require_voice!(voice_dir);
    let d = load(&voice_dir, "colligation.pkg");
    let mut seen = vec![false; d.n1()];
    let mut stack = vec![1usize];
    seen[1] = true;
...
fn nonreg_reversed_keys_round_trip() {
    require_voice!(voice_dir);
    let d = load(&voice_dir, "NonReg.pkg");
    let mut parent: Vec<Option<(usize, u8)>> = vec![None; d.n1()];
    parent[1] = Some((0, 0));
    let mut stack = vec![1usize];
    let mut terms = Vec::new();
...
fn malformed_input_rejected_and_search_bounds() {
    require_voice!(voice_dir);
    let good = std::fs::read(voice_dir.join("User.pkg")).unwrap();
...
fn conjects_x_are_connect_blob_indices() {
    require_voice!(voice_dir);
    let d = load(&voice_dir, "Conjects.pkg");
    let terms = reachable_terminals(&d);
